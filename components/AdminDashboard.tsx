...
type AdminTab = 'Dashboard' | 'Members' | 'Finances' | 'Assignments' | 'Notices';
      ...
      {selectedUser && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-2xl bg-white rounded-[3rem] shadow-2xl overflow-hidden p-10 max-h-[90vh] overflow-y-auto custom-scrollbar">
             ...
             <div className="space-y-4">
                <h5 className="text-sm font-black text-slate-900 uppercase tracking-widest ml-2">Recent Learning Logs</h5>
                <div className="space-y-2">
                   {['AI 탐구 보고서 - 기후 위기', '영단어 마스터 테스트 #12', '자기주도 학습 일지'].map((task, i) => (
                     <div key={i} className="p-5 bg-slate-50 border border-slate-100 rounded-2xl flex justify-between items-center group hover:bg-white hover:border-blue-200 transition-all">
                        <span className="text-xs font-bold text-slate-700">{task}</span>
                        <span className="text-[9px] font-black text-green-500 uppercase tracking-widest bg-green-50 px-3 py-1 rounded-full">Completed</span>
                     </div>
                   ))}
                </div>
             </div>
