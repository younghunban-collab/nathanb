...
const PointShop: React.FC<PointShopProps> = ({ lang, user }) => {
  const [activeCategory, setActiveCategory] = useState<'all' | 'avatar' | 'goods' | 'coupon'>('all');

  const userPoints = user?.points ? `${user.points.toLocaleString()} MP` : "45,800 MP";

  const handleBuy = (itemName: string, price: string) => {
    const confirmBuy = window.confirm(
      lang === 'ko' 
        ? `${itemName}을(를) ${price}MP에 구매하시겠습니까?` 
        : `Do you want to buy ${itemName} for ${price}MP?`
    );
    if (confirmBuy) {
      alert(lang === 'ko' ? "구매가 완료되었습니다! 인벤토리나 쿠폰함을 확인해주세요." : "Purchase complete! Check your inventory or coupons.");
    }
  };
        ...
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row justify-between items-center relative z-10 gap-16">
          <div className="space-y-8 text-center lg:text-left">
            ...
            <p className="text-slate-400 max-w-md mx-auto lg:mx-0 text-lg italic leading-relaxed">
              {lang === 'ko' ? <>"열심히 공부한 당신, <br /> 쌓인 MP로 나만의 가치를 구매하세요."</> : <>"Hard-working you, <br /> purchase your value with accumulated MP."</>}
            </p>
          </div>
          ...
          <div className="bg-white/5 backdrop-blur-xl p-8 lg:p-12 rounded-[3.5rem] border border-white/10 shadow-2xl animate-in zoom-in duration-700 min-w-[320px]">
             <p className="text-[11px] font-black text-blue-400 uppercase tracking-widest mb-4 opacity-80">{lang === 'ko' ? '내 잔액' : 'My Current Balance'}</p>
             ...
             <div className="mt-8 pt-6 border-t border-white/10">
                <p className="text-[10px] text-slate-500 font-bold italic tracking-tighter">
                  {lang === 'ko' ? '* 포인트는 학습 성과에 따라 매일 자동 지급됩니다.' : '* Points are automatically paid daily based on learning performance.'}
                </p>
             </div>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-6 -mt-16 relative z-20">
        <div className="bg-white p-5 rounded-[3rem] shadow-2xl border border-slate-100 flex flex-wrap gap-4 items-center">
           ...
           <div className="flex-1 lg:flex-none flex items-center gap-3 bg-slate-50 px-6 py-4 rounded-3xl border border-slate-100 focus-within:ring-2 ring-blue-600/10 transition-all">
              ...
              <input type="text" placeholder={lang === 'ko' ? '아이템 검색...' : 'Search items...'} className="bg-transparent text-sm outline-none w-full lg:w-40 font-bold" />
           </div>
        </div>
      </section>
      ...
      <section className="max-w-7xl mx-auto px-6 mt-24">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-16 gap-6">
           <div className="space-y-2">
              ...
              <p className="text-slate-400 text-sm font-medium">{lang === 'ko' ? '오늘 가장 인기 있는 아이템들을 확인해보세요.' : 'Check out today\'s most popular items.'}</p>
           </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-10">
           {filteredItems.map((item) => (
             <div key={item.id} className="group bg-white p-7 rounded-[3.5rem] shadow-xl shadow-slate-200/40 border border-slate-50 hover:shadow-2xl hover:border-blue-100 transition-all duration-500 relative flex flex-col h-full">
                ...
                <div className={`aspect-square rounded-[2.5rem] mb-8 overflow-hidden relative shadow-inner ${item.type === 'Avatar' ? 'mix-blend-multiply' : ''}`}>
                ...
                <div className="flex items-center justify-between pt-6 border-t border-slate-50 mt-auto">
                   ...
                   <button 
                    ...
                    className="w-14 h-14 bg-slate-900 text-white rounded-[1.5rem] flex items-center justify-center hover:bg-blue-600 hover:scale-110 transition-all shadow-xl active:scale-90 group-hover:rotate-6"
                   >
      ...
      <section className="max-w-7xl mx-auto px-6 mt-40">
         <div className="bg-gradient-to-br from-blue-700 via-blue-600 to-indigo-800 rounded-[5rem] p-16 lg:p-24 flex flex-col lg:flex-row items-center justify-between gap-16 relative overflow-hidden shadow-2xl shadow-blue-600/40 border border-white/10">
            <div className="space-y-10 relative z-10 text-white text-center lg:text-left">
               ...
               <p className="text-xl text-blue-100 italic max-w-lg leading-relaxed">
                 {lang === 'ko' ? '"한정판 전설 아바타가 매주 월요일 오후 6시에 시크릿 드랍됩니다. 기회를 놓치지 마세요!"' : '"Limited edition legendary avatars drop every Monday at 6 PM. Don\'t miss the chance!"'}
               </p>
               <button 
                onClick={() => alert(lang === 'ko' ? "차주 월요일 오후 6시에 알림이 전송됩니다!" : "Reminder set for next Monday 6 PM!")}
                className="px-12 py-6 bg-white text-slate-900 rounded-[2rem] font-black uppercase tracking-widest hover:bg-slate-900 hover:text-white transition-all shadow-2xl transform hover:-translate-y-1"
               >
                Check Drop Schedule ↗
               </button>
            </div>
